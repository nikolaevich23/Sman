...
static int32_t set_dec_param(jpg_dec_info	*dec_ctx)
{
	...
	if(!jpg_w || !jpg_h || (jpg_w!=1920 && (jpg_w*jpg_h)>(MAX_WH4*2))) return -1;
	...
	if(jpg_w!=1920 && ((jpg_w*jpg_h*4)>(MAX_WH4) || jpg_w>330 || jpg_h>330 || gpp==40))
	{
		if(gpp==10)
		{
			in.downScale = 2;
...
Buffer load_jpg(const char *file_path, void* buf_addr)
{
	...
	tmp.w=tmp.h=tmp.x=tmp.y=tmp.b=jpg_w=jpg_h=0;
