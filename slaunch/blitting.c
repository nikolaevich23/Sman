...
void dim_img(float dim, uint32_t x, uint32_t y, uint32_t w, uint32_t h)
{
	...
	for(i = 0; i < h ; i++)
		...
		{
			new_pixel = *(uint64_t*)(OFFSET(k*2+x, (i+y)));
			...
			*(uint64_t*)(OFFSET(k*2+x, (i+y))) = new_pixel_R1<<48 | new_pixel_G1<<40 | new_pixel_B1<<32 | new_pixel_R0<<16 | new_pixel_G0<< 8 | new_pixel_B0;
		}
}
...
void init_graphic()
{
	...
	ctx.pixels     = mem_alloc(CANVAS_W * CANVAS_H * 4);  //
	ctx.menu	   = mem_alloc(CANVAS_W * 96 * 4);
	ctx.font_cache = mem_alloc(FONT_CACHE_MAX * 32 * 32); // glyph bitmap cache
	ctx.imgs	   = mem_alloc(MAX_WH4);				  // images (actually just 1 image with max 336x336 resolution)
	ctx.side	   = mem_alloc(SM_M);
	...
	for(uint32_t y=0;y<CANVAS_H; y++)
		for(uint32_t x=0;x<CANVAS_W; x++)
			ctx.pixels[x+y*CANVAS_W]=( (uint32_t) ( BASE_offset + (disp_h==1080?((	( ((x)<<2) + (y) * 8192) )):(disp_h==720?((	(( ((((x)* 2)/3)<<2) + (((y)* 2)/ 3) * 5120) ) & 0xfffffff8)):(disp_h==576?((	(( ((((x)* 3)/8)<<2) + (((y)* 8)/15) * 3072) ) & 0xfffffff8)):((	(( ((((x)* 3)/8)<<2) + (((y)* 4)/ 9) * 3072) ) & 0xfffffff8))))) ) );
...
int32_t load_img_bitmap(int32_t idx, const char *path)
{
	...
	if(ctx.img[idx].w<=(MAX_W/2) && ctx.img[idx].h<=(MAX_H/2) && gpp==10)
	{
		//upscale x2
