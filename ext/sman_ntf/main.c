...
SYS_MODULE_INFO(RAWSECISO, 0, 1, 1);
SYS_MODULE_START(rawseciso_start);
SYS_MODULE_STOP(rawseciso_stop);

#define THREAD_NAME "sm_ntfm"
#define STOP_THREAD_NAME "sm_ntfs"

#define MIN(a, b)	((a) <= (b) ? (a) : (b))
#define ABS(a)		(((a) < 0) ? -(a) : (a))

#define CD_CACHE_SIZE			(32)
...
static int process_read_cd_2048_cmd(uint8_t *buf, uint32_t start_sector, uint32_t sector_count)
{
	...
	uint32_t fit = capacity/cd_sec_size;
	...
	if (fit > 0)
	{
		process_read_iso_cmd(buf, start_sector*cd_sec_size, fit*cd_sec_size);
	...
	for (i = 0; i < rem; i++)
	{
		process_read_iso_cmd(out, (start_sector*cd_sec_size)+24, 2048);
		out += 2048;
		start_sector++;
	}
...
static int process_read_cd_2352_cmd(uint8_t *buf, uint32_t sector, uint32_t remaining)
{
	...
	if (process_read_iso_cmd(cd_cache, sector*cd_sec_size, CD_CACHE_SIZE*cd_sec_size) != 0)
		return -1;
...
static void rawseciso_thread(uint64_t arg)
{
	...
	else if(!(discsize%2368)) cd_sec_size = 2368;
	...
	if (is_cd2352)
	{
		if (discsize % cd_sec_size)
			discsize = discsize - (discsize % cd_sec_size);
	}
	...
	ret = sys_storage_ext_mount_discfile_proxy(result_port, command_queue, emu_mode, discsize, 128*1024, num_tracks|(cd_sec_size<<4), tracks);
	//DPRINTF("mount = %x\n", ret);
